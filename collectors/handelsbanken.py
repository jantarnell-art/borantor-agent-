...
class HandelsbankenCollector(BaseCollector):
    ...
    def collect_list_rates(self) -> list[RateResult]:
        ...
        if results:
            logger.info("Handelsbanken: collected %d list rates", len(results))
        else:
            logger.warning("Handelsbanken: no rates parsed")
