...
class SkandiaCollector(BaseCollector):
    bank_name = "Skandia"
    ...
    def collect_list_rates(self) -> list[RateResult]:
        ...
        if results:
            logger.info("Skandia: collected %d list rates", len(results))
        else:
            logger.warning("Skandia: no rates parsed")
