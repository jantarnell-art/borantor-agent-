...
class DanskeBankCollector(BaseCollector):
    ...
    def collect_list_rates(self) -> list[RateResult]:
        ...
        if results:
            logger.info("Danske Bank: collected %d list rates", len(results))
        else:
            logger.warning("Danske Bank: no rates parsed")
