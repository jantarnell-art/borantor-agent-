...
class SwedbankCollector(BaseCollector):
    bank_name = "Swedbank"
    list_rates_url = (
        "https://www.swedbank.se/privat/bolan-och-bostadslan/"
        "rantor-och-villkor/aktuella-bolanerantor.html"
    )
    ...
    def collect_list_rates(self) -> list[RateResult]:
        ...
        if results:
            logger.info("Swedbank: collected %d list rates", len(results))
        else:
            logger.warning("Swedbank: no rates parsed – page structure may have changed")
